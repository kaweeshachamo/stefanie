...
Stefanie.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN}, (async (message, match) => {
    if (match[1] === '') return await message.sendMessage(Lang.NEED_URL + '.install https://gist.github.com/Kaweeshachamodk/4232b1c8c4734e1f06c3d991149c6fbd')
    try {
    ...
    if (response.statusCode == 200) {
        ...
        try {
            ...
            fs.unlinkSync('/root/stefanie/plugins/' + plugin_name + '.js')
        ...
        if (!match[1].includes('Kaweeshachamodk')) {
...
Stefanie.addCommand({pattern: 'pluglist', fromMe: true, desc: Lang.PLUGIN_DESC }, (async (message, match) => {
    ...
    if (plugins.length < 1) {
        ...
        plugins.map(
            (plugin) => {
                let vf = plugin.dataValues.url.includes('Kaweeshachamodk') ? msg : inmsg
